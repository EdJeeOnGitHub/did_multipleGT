...
did_multipleGT_check <- function(data,
  ...
  if (max(n_by_group) > 1 & !is.null(weight)) {
  ...
  if (unique(na.omit(data$time_XX)) < 2) {
  ...
  d_cat_group_XX <- with(data, ave(rep(1, nrow(data)), D_cat_XX, FUN = seq_along))
  if (unique(d_cat_group_XX) == 1) {
